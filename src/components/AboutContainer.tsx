...
const AboutContainer: React.FC<AboutContainerProps> = ({className, title, description, imgSrc, imgClassName, imgStyle, date}) => {
    return (
        <div className={"about-container flex flex-col items-start relative text-gray-800 text-left bg-white rounded-lg shadow-md p-4 border-l-4 border-red-400 pr-20  transition-all duration-300 sm:hover:scale-105 transform " + className}>
            ...
            <h2 className="about-title text-red-400 sm:text-lg ">{title}</h2>
            <p className="about-description text-xs sm:text-sm">{description}</p>
            <p className="about-date text-xs sm:text-sm text-gray-600">{date}</p>
        </div>
    );
}
